...
class Miniskirt < Struct.new(:__klass__)
  ...
  class << self
    ...
    def build name, attrs = {}
      ...
      raise NoFactoryError, "Something went terribly wrong, you may not have defined the factory for ':#{name}', or forgot to load it." if h.nil?
      ...
      (m = m.is_a?(Class) ? m : m.to_s.camelize.constantize).new.tap do |r|
        ...
        attrs.symbolize_keys!.reverse_update(h).each do |k, v|
          r.send "#{k}=", case v when String # Sequence and interpolate.
            ...
            v.sub(/%\d*d/) { |d|
              d % n ||= (m.respond_to?(:maximum) ? m.maximum(:id) : m.max(:id)).to_i + 1
            } % attrs % n

          when Proc then v.call(r) else v end
        end
      end
    end
